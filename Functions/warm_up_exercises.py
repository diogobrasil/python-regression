...
def warm_up_exercise3(x):
    ...
    m = x.shape[0]
    # Garante que x é um vetor coluna usando reshape
    x = x.reshape((m, 1))
    # Adiciona uma coluna de 1s (bias) ao vetor x
    bias = np.ones((m, 1))
    # Concatena a coluna de 1s (bias) com o vetor x
...
def warm_up_exercise5(predictions, y):
    ...
    """
    # Calcula o vetor de erros quadráticos
